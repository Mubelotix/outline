...
export default function auth(options: AuthenticationOptions = {}) {
  return async function authMiddleware(ctx: AppContext, next: Next) {
    ...
    let type: AuthenticationType | null = null;
    ...
    if (
      ...
      const { user: user2, token: token2, type: type2 } = await validateAuthentication(ctx)
      user = user2; token = token2; type = type2;
    }
    ...
    try {
      ...
      await validateAuthenticationSecondStage(user, type!, options);
...
async function validateAuthenticationSecondStage(
  user: User,
